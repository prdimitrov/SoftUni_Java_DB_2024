...
//                .stream().map(Book::getTitle).collect(Collectors.toList());
    ...
    public List<String> findAllTitlesByEditionAndCopies(EditionType type, int copies) {
        ...
        return bookRepository.findByEditionTypeAndCopiesLessThan(type, copies)
                .stream()
                .map(Book::getTitle)
    ...
    @Override
    public List<Book> findByPriceLessThanOrPriceGreaterThan(float minPrice, float maxPrice) {
        BigDecimal lower = BigDecimal.valueOf(minPrice);
        BigDecimal higher = BigDecimal.valueOf(maxPrice);
        return bookRepository.findByPriceLessThanOrPriceGreaterThan(lower, higher);
    }

    @Override
    public List<String> findNotReleasedIn(int inputYear) {

        return this.bookRepository.findByReleaseDateYearNot(inputYear);
    }

    @Override
    public List<Book> findBooksReleasedBefore(String inputDate) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        LocalDate before = LocalDate.parse(inputDate, formatter);
